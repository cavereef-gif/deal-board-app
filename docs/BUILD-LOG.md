...
- Calculators (More › Calculators, or + › Calculators) in three tabs: Transport (from/to → road distance from OpenStreetMap, map, km there and back, rate per km, tolls, tons per load, client rate → cost per ton, left per ton, per load, per month; optional diesel), Chrome and ore (the old deal calculator), Everyday (keypad, + or − 15% VAT, earlier sums). Numbers show as R 39 720 and 12.5 on every phone.
