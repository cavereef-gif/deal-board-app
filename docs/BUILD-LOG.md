...
- The bot (ask v10, deployed 26 Sep; still Claude Haiku) has a new app_action tool, used only by the v17 app (it sends app:2): open / show / calculator happen at once; changes come back as cards – "Move … to Mon 28 Sep", "Make … urgent", "Give … to Annemarie", "Follow up Tue 29 Sep", "Tick the NCNDA step", "Client rate R520/t", deal status, a new deal, a board message, accept / drop / done – each with Do it and No. Nothing is saved until Do it is tapped. The private target and walk-away numbers are refused (in the function and in the app). The live app (v14) and preview (v16) do not send app:2 and get exactly the v9 tools. New tasks still arrive as Suggested and can carry a due date.
...
- Buyer-search steps: the "What happened?" form now has Done · Follow up · Blocked · Drop in two equal columns, plus a "No reply yet? Follow up on" date. The date is taken from the step when it says one ("if nothing by 29 Sep"), otherwise 3 days ahead. A follow-up stays open, shows "Follow up Tue 29 Sep" and the last note, and appears on Today under its day; tapping it opens the step with the form ready.
- Today: four tiles on top in a 2 × 2 grid – Urgent (red) · Overdue (amber) · Today (blue) · This week (periwinkle). Tap one to see only those, tap again (or Show everything) for the full list. Every group is now one card with a coloured dot, title and count: Suggested (grey, with Accept all) · Overdue · Today · Tomorrow (cyan) · Next 7 days · Later (folded) · Next step on each deal · Buyer search · Risks. Row rails still show priority. Words stay neutral.
- Transport calculator: the answer first (Transport cost · Left for you · Left a month, green rail when you make money, red when you lose), then three numbered steps – 1 Route · 2 Truck · 3 Client – with no drop-down lists: "There and back | One way only" is a bar, deals are buttons, Save goes to the chosen deal or the board. Diesel is folded away as optional. "How it adds up" keeps the full sum for Save and Copy.
- Checks: 184 screens with the real font – all readability rules pass, lowest contrast 5.03, no coloured text, no bold at 20px+; the only sub-40px items are two tick boxes that sit inside 48px tap rows (unchanged). Lead links on a step raised from 30px to 40px. flows.py: 36 jobs PASS (new: tiles filter, Follow up keeps a step open and dated and puts it on Today, tapping it opens the form, One way only halves the km with no drop-downs). Screenshots looked at in Titanium and Velvet, night and day, both sizes.
- Commits: prototype branch e803f75; prototype link updated by 51735b4 on main (prototype/ folder only). Live app (v14) and preview (v16) untouched.
