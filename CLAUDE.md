...
- Minimum sizes and the rest of the look rules are in docs/PLAN.md batch 1. Colours: "Graphite" (titanium.css, html[data-palette="titanium"], the default since 26 Sep 2026; blue only on main buttons, a colour per section) and "Velvet" (tokens in app.css :root and [data-theme="light"]) – keep both; Settings switches. Controls are 40px (main buttons 48px).

## How the app must behave (standing rules)
- Claude proposes, people confirm: the bot never ticks checklist steps and never confirms, completes or deletes anything by itself. Anything automatic arrives as "Suggested". Since 26 Sep 2026 (Chris: "it prepares, I tap once") the bot may open and show things at once, and may prepare a change as a "Do it" card – nothing is saved until a person taps Do it.
